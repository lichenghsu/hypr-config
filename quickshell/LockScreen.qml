    ...
    Variants {
        ...
        PanelWindow {
            ...
            Item {
                id: crtLayer
                anchors.fill: parent
                layer.enabled: lockRoot.active || lockRoot.preLockActive
                layer.effect: ShaderEffect {
                    property variant source
                    property real time: 0.0
                    property real curvature: 0.12
                    property real vignetteStrength: 0.55
                    property real scanlineStrength: 0.12
                ...
                ShaderEffect {
                    ...
                    property color matrixTint: {
                        if (lockRoot.intrusionActive) {
                            var p = lockRoot.intrusionPhase
                            if (p === 0) return "#ff1133"
                            if (p === 1) return "#ff6600"
                    ...
                    property real tintB: matrixTint.b
                    Behavior on tintR { NumberAnimation { duration: 220 } }
                    Behavior on tintG { NumberAnimation { duration: 220 } }
                    Behavior on tintB { NumberAnimation { duration: 220 } }
